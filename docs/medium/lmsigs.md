...
Symbolically, we may denote a transaction ```T = (I, O, f, A)```. In practice, ```A``` generally consists of a multi-signature with the keys in the inputs in ```I``` on the optional memo field message (which is assumed to be the zero bit if it is otherwise empty). Users can broadcast these transactions on any network however they like, and validators can assemble valid transactions into a sensible transaction history using whatever consensus mechanism they like.
