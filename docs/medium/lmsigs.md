...
Let ```g``` be a generator of an elliptic curve group ```G``` of order ```p``` over which the discrete logarithm game is hard, and assume ```g``` is a group element sampled uniformly at random. Let ```F``` be a random oracle function that outputs elements from the integers modulo ```p```. The following defines a one-time signature scheme that satisfies one-time existential unforgeability.
  ...
  1. ```Keygen -> (sk, vk)```. Sample ```r, s``` uniformly at random and independently from the non-zero integers modulo ```p```, set ```x = g ** r, y = g ** s```. Set ```sk = (r, s)``` and ```vk = (x, y)```. Output ```(sk, vk)```.
...
In [](), we present a library that implements these signatures, which we call LMSigs after Lyubashevsky and Micciancio; their work in []() is foundational for this scheme, which was expanded upon by Boneh and Kim in []().

### Transaction Protocols And Usual Digital Signatures

For the purposes of this discussion, a _transaction_ is a tuple ```(I, O, fee, A, memo)``` where ```I``` is a set of _inputs_, ```O``` is a set of _outputs_, ```fee``` is a plaintext fee, ```A``` is an authentication of some sort, and ```memo``` is an optional memo field.  A transaction protocol consists of a public bulletin board upon which transactions are posted, together with some sort of map or lookup called ```Amount``` that maps inputs and outputs to some plaintext transaction amount. A transaction posted on the bulletin board is considered _valid_ if every input reference points to a valid transaction, the signatures in the authentication ```A``` are valid, and ```Amount(I) - Amount(O) - fee = 0```.

Symbolically, we may denote a transaction ```T = (I, O, f, A)```. In practice, ```A``` generally consists of a multisignature with the keys in the inputs in ```I``` on the optional memo field message (which is assumed to be the zero bit if it is otherwise empty). Users can broadcast these transactions on any network however they like, and validators can assemble valid transactions into a sensible transaction history using whatever consensus mechanism they like.

Thus, to nail down a transaction protocol generally only requires nailing down the structure of ```I```, ```O```, and ```Amount```. We can construct a transaction protocol out of a usual digital signature scheme quite easily. Indeed, each input in ```I``` consists of (i) an unambiguous reference to an output from a previous transaction and (ii) a plaintext amount. Each output in ```O``` consists of (i) a public verification key and (ii) a plaintext amount. Note that this means that each input references a verification key, because each input references an output in an old valid transaction. The authentication in ```A``` consists of one signature on the memo field from each verification key found in each input in ```I```.  The ```Amount``` function basically just looks at the plaintext amounts. 
...
However, this approach is foiled when we start considering one-time signatures.

### Transaction Protocols And One-Time Signatures

We can use one-time signatures in a transaction protocol, if we like, but we need some sort of key exchange. This way, we can ensure that the recipient can extract the new one-time signing keys associated with their new outputs, whilst ensuring that the sender cannot extract that key.

#### Example: Monero/Cryptonote-Style One-Time Keys

Cryptonote used the computational Diffie-Hellman assumption and hash functions in order to construct one-time keys from many-time wallet keys. We show how this key exchange can be used to develop a transparent version of Cryptonote.

In the following, let ```(G, p, g, F)``` be the group parameters from our Schnorr-like example of one-time signatures.

  0. In a setup phase, users agree upon ```(G, p, g, F)```, and another hash function ```H``` whose codomain is the integers modulo ```p```.
  1. Users select many-time wallet keys by sampling ```(a, b)``` uniformly at random and independently from the integers modulo ```p```. Users set ```A = g**a```, ```B = g**b```, ```sk = (a, b)```, and ```vk = (A, B)```. Users then broadcast ```vk```.
  2. An output is a tuple ```(R, P, amt)``` where ```R``` and ```P``` are group elements and ```amt``` is a plaintext amount.
  3. To send an amount ```amt``` to another user with some ```vk = (A, B)```, a new output ```(R, P, amt)``` is constructed in the following way. First, we sample a new random ```r``` uniformly at random and independently from all previous samples from the integers modulo ```p```, and we set ```R = g**r```. Next, we use ```r``` and the recipient ```A``` to compute the Diffie-Hellman shared secret ```A ** r = R ** a```. We hash this with ```H``` and compute ```P = g ** H(A ** r) * B```. 
...
This leads to one-time keys in the sense that only one signature ever needs to be posted in order for a transaction to be considered valid and complete. However, depending on the properties of the underlying signature scheme, it may or may not be safe to publish more than one signature from a key ```P```. Note that to determine whether a transaction is addressed to ```(A, B)```, we only need the private key ```a```, which is why we call it a _view key_. On the other hand, we call ```b``` a _spend key_ because we need it to construct new transactions. The overall scheme itself is a rather inelegant instantiation of a key encapsulation mechanism, and the one-time-ness of the protocol can be a "soft" one-time-ness. By using a proper key encapsulation mechanism and one-time signatures, we can make this into a proper one-time protocol.
