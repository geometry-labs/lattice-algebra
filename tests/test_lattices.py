...
def test_decode2indices(secpar, dist, lp, dist_pars, val, num_coefs, expected_output):
    observed_output = decode2indices(secpar=secpar, lp=lp, num_coefs=dist_pars['wt'], val=val,
...
@pytest.mark.parametrize(
    "secpar,lp,dist,dist_pars,val,num_coefs,bits_to_indices,bits_to_decode,expected_indices,expected_coefs,expected_output",
    DECODE2POLYCOEFS_CASES)
...
exp_bits_per_poly_for_testing: int = small_dist_pars['wt'] * bits_to_decode(secpar=secpar4testing,
                                                                            bd=small_dist_pars['bd']) + bits_to_indices(
